...
public interface UserRepo extends CrudRepository<User, Long> {
    ...
    @Query(value = "SELECT username, score FROM scores JOIN users u on scores.user_id = u.id  WHERE date_of_game LIKE ?1 AND scores.id > (SELECT MIN(id) FROM scores) ORDER BY score DESC LIMIT 10;", nativeQuery = true)
